...
| `edge-box/` | **边缘盒 C++ 端**：多路视频/推理后端抽象/Mock+ONNX/IOU跟踪/虚拟线客流/匿名轨迹/断网缓存 | ✅ 骨架（待部署机编译） |
...
cd edge-box && cmake -B build && cmake --build build && ctest --test-dir build  # C++ smoke（需部署机）
```
...
| C++（边缘盒） | CMake ≥ 3.16，可选 OpenCV 4 / ONNXRuntime / SQLite3 / cpp-httplib |

## 待办（Roadmap）

- [ ] C++ 端 ONNX 完整推理（SCRFD decode/NMS、ArcFace 预处理、活体）
- [ ] C++ 端 SQLite 存储 + cpp-httplib 上报 + 人员库增量同步
- [ ] 录入电脑端（C++ Qt / 身份证读卡器 SDK）
- [ ] 前端客流图表可视化（ECharts 趋势）
- [ ] 生产部署：MySQL 接入、对象存储（MinIO/S3）、HTTPS
